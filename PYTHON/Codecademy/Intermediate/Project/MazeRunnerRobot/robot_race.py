...
# Add your code below!
num_of_collision = Counter(move[0] for move in robot_moves if move[2] == True)
...
# Add your code below!
for bot in bots:
    bot_scores.append(BotScoreData(bot.name, num_of_moves[bot.name], num_of_collision[bot.name],
                                    num_of_moves[bot.name] + num_of_collision[bot.name]))
...
while len(robot_moves) > 0:
    ...
    # Add your code below!
    bot_names, direction, has_collided = robot_moves.popleft()
    bot_data[bot_names].process_move(direction)
